...
public class FileBufferReader implements BufferReader {
    ...
    private FileBufferReader(Path path) throws IOException {
        this.filePath = path;
    ...
    public String getBuffer(long n) {
        ...
        if (softRef != null) {
            return NullnessUtil.castNonNull(softRef.get());
        }
    ...
    public static FileBufferReader fromFile(Path path) throws IOException {
        return new FileBufferReader(path);
    }
}
